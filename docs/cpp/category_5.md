...
```
### HDEX_InstallPlugin1 - 基础示例
```cpp
//先安装再打开
auto ret = HDEX_InstallPlugin1(1,"Game.exe",64,1,0,0,FALSE, (__int64)打开回调1, (__int64)检查回调);
//回调
...
//新窗口触发
__int64 __stdcall 打开回调1(int windowsIndex)//返回PID错误值
{
	//return HCSYS_RunAppExxxx("D:\\XTLBB\\Bin64\\", "Game.exe", "-fl", 0, 0);
	return HCSYS_RunAppExxxx("M:\\ZXSJclient\\ZXSJLaunch\\", "ZXSJGame.exe", "", 0, 0);
}
//HCInject_GetPrePid  大于0的值
//内部检查到
//1.当前对应的窗口序号 有绑定过PID --重连窗口
__int64 __stdcall 检查回调(int 窗口序号, int 之前窗口序号, int 之前进程PID, int 提示值)
{
	if (提示值== RET_重连窗口)
	{
		//1.1
		return 1;//重连  方便调试
		//1.2
		HANDLE handle = OpenProcess(PROCESS_ALL_ACCESS, FALSE, 之前进程PID);
		if (::TerminateProcess(handle, -1))
		{
			return 0;//新窗口
		}
		return -1;//自定义值 小于0 //中断
	}
	if(提示值== RET_检查PID存在)
	{
		return 0;//一般情况下返回0就行 除非需要单独处理PID
	}
	if (提示值 == RET_重连窗口序号不一致)
	{
		//之前绑定的序号或者错误值(小于0)
		//代表PID
		if (之前窗口序号 >0)
		{
			...
			return -2;//自定义值 小于0 中断
			//别的处理方式
			//return 1;//1重连并使用之前绑定的序号进行加载环境  之前窗口序号 重连  之前进程PID
		}
		//错误值 小于等于0
		else if (之前窗口序号 == ERROR_进程不存在)
		{
			return 0;//新窗口
		}
		//错误值 小于等于0
		else if (之前窗口序号 == ERROR_未安装插件)
		{
			return 0;//新窗口
		}
		return 之前窗口序号;
...
```
### HDEX_InstallPlugin1 - 进阶用法
```cpp
// 示例2：HDEX_InstallPlugin1 调用
//先安装再打开
auto ret = HDEX_InstallPlugin1(1,"Game.exe",64,1,0,0,FALSE, (__int64)打开回调1, (__int64)检查回调);
//回调
#define RET_检查PID存在 8
...
```
### HDEX_InstallPlugin1 - 高级应用
```cpp
// 示例3：HDEX_InstallPlugin1 高级用法
//先安装再打开
auto ret = HDEX_InstallPlugin1(1,"Game.exe",64,1,0,0,FALSE, (__int64)打开回调1, (__int64)检查回调);
//回调
#define RET_检查PID存在 8
...
```
### HDEX_InstallPlugin2 - 基础示例
```cpp
// C++示例代码
// 这里可以放置与说明内容相关的代码示例
代码内容;
```
### HDEX_InstallPlugin2 - 进阶用法
```cpp
// 示例2：HDEX_InstallPlugin2 调用
// [示例2] C++示例代码
// [示例2] 这里可以放置与说明内容相关的代码示例
代码内容;
```
### HDEX_InstallPlugin2 - 高级应用
```cpp
// 示例3：HDEX_InstallPlugin2 高级用法
// [高级] C++示例代码
...
几个核心的点归纳总结出来其实就能规避掉很多坑了比如1.没安装过插件的都不算重连窗口全部当新窗口处理会触发打开回调2.安装过插件的不管是卸载了环境还是崩溃了中控都算重连窗口不会触发打开回调
```
### HDEX_InstallPlugin3 - 基础示例
```cpp
// C++示例代码
// 这里可以放置与说明内容相关的代码示例
代码内容;
```
### HDEX_InstallPlugin3 - 进阶用法
```cpp
// 示例2：HDEX_InstallPlugin3 调用
// [示例2] C++示例代码
// [示例2] 这里可以放置与说明内容相关的代码示例
代码内容;
```
### HDEX_InstallPlugin3 - 高级应用
```cpp
// 示例3：HDEX_InstallPlugin3 高级用法
// [高级] C++示例代码
// [高级] 这里可以放置与说明内容相关的代码示例
代码内容;
```
...
几个核心的点归纳总结出来其实就能规避掉很多坑了比如1.没安装过插件的都不算重连窗口全部当新窗口处理会触发打开回调2.安装过插件的不管是卸载了环境还是崩溃了中控都算重连窗口不会触发打开回调
```
### HDEX_InstallPluginVM - 基础示例
```cpp
// C++示例代码
// 这里可以放置与说明内容相关的代码示例
代码内容;
```
### HDEX_InstallPluginVM - 进阶用法
```cpp
// 示例2：HDEX_InstallPluginVM 调用
// [示例2] C++示例代码
// [示例2] 这里可以放置与说明内容相关的代码示例
代码内容;
```
### HDEX_InstallPluginVM - 高级应用
```cpp
// 示例3：HDEX_InstallPluginVM 高级用法
// [高级] C++示例代码
// [高级] 这里可以放置与说明内容相关的代码示例
代码内容;
```
